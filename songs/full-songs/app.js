...
function values() {
	return {
		...
		lyrics: normalizeLyricsText(ui.lyrics.value),
...
function showSong(data) {
	if (!data.audioUrl) {
		throw new Error("ACE-Step did not return audio.");
	}
	...
	ui.info.textContent = data.info
		? `ACE-Step info: ${JSON.stringify(data.info)}`
...
async function generateLyrics(includeExisting) {
	...
	try {
		...
		ui.lyrics.value = normalizeLyricsText(data.lyrics || current.lyrics);
		ui.info.textContent =
...
async function makeSong() {
	...
	setBusy(true, "Making song with ACE-Step...");
	try {
...
async function checkHealth() {
	try {
		...
		if (data.ok && data.provider === "hf-spaces") {
			setHealth("ready", "ACE-Step ready");
			return;
		}
		setHealth("error", "Worker not ready");
	} catch {
