...
class Game {
    ...
    init() {
        ...
        this.addLog('Round 1 briefing: enemy formation detected. Scout before committing your lineup.', 'info');
        this.updateUI();
    ...
    bindControls() {
        ...
        byId('start-battle-btn').addEventListener('click', () => this.startBattle());
        byId('next-round-btn').addEventListener('click', () => this.nextRound());
        byId('skip-scout-btn').addEventListener('click', () => this.endScoutPhase());
        byId('sell-unit-btn').addEventListener('click', () => this.sellSelectedUnit());
        byId('auto-arrange-btn').addEventListener('click', () => this.autoArrangeArmy());
        byId('lock-shop-btn').addEventListener('click', () => this.toggleShopLock());
        byId('pause-battle-btn').addEventListener('click', () => this.toggleBattlePause());
    ...
    getTeamUnits(team, aliveOnly = true) {
        return this.units.filter(unit => unit.team === team && (!aliveOnly || unit.isAlive()));
    }

    getUnitAt(x, y) {
        return this.units.find(unit => unit.isAlive() && unit.x === x && unit.y === y) || null;
    }
    ...
    summarizeTeam(team) {
        ...
        const threat = Math.round(units.reduce((sum, unit) => (
            sum +
            unit.maxHp * 0.12 +
            unit.damage * 3.4 +
            unit.range * 6 +
        ...
        switch (this.phase) {
            case PHASES.SCOUT:
                return {
                    ...
                    description: 'Enemy positions are fully revealed before your shop and deployment open.',
            ...
            case PHASES.PREP:
                return {
                    ...
                    description: 'Recruit, reposition, sell, auto-arrange, and lock the shop before combat begins.',
            ...
            default:
                return {
                    ...
                    description: 'Collect your payout and move survivors into the next wave.',
    ...
    endScoutPhase() {
        ...
        this.addLog('Scout phase complete. Command of ally placement and shop controls is now live.', 'info');
        this.updateUI();
    }
    ...
    rerollShop(isManual = false, force = false) {
        ...
        if (isManual) {
            ...
            this.shopUnits = this.shopUnits.map(unitKey => unitKey || randomShopUnit());
            this.selectedShopIndex = null;
            this.addLog('Shop lock held your saved offers and refreshed any empty slots.', 'info');
            this.updateUI();
            return;
        }
        this.shopUnits = [];
    ...
    handleCellClick(x, y) {
        ...
        if (selectedUnit && selectedUnit.team === 'player' && this.canDeployTo(x, y)) {
    ...
    deploySelectedShopUnit(x, y) {
        ...
        this.addLog(`Deployed ${newUnit.name} to ${this.getCellLabel(x, y)}.`, 'success');
        this.updateUI();
    }
    ...
    moveSelectedUnitTo(x, y) {
        ...
        unit.x = x;
        unit.y = y;
        this.addLog(`Repositioned ${unit.name} from ${from} to ${this.getCellLabel(x, y)}.`, 'info');
        this.updateUI();
    }
    ...
    autoArrangeArmy() {
        ...
        const poolByBand = {
            front: buildSlots([PLAYER_ZONE_START, PLAYER_ZONE_START + 1], columnPriority),
            mid: buildSlots([PLAYER_ZONE_START + 1, PLAYER_ZONE_START + 2], columnPriority),
            back: buildSlots([GRID_SIZE - 2, GRID_SIZE - 1], columnPriority)
        };
        const fallbackSlots = buildSlots(
            [PLAYER_ZONE_START, PLAYER_ZONE_START + 1, PLAYER_ZONE_START + 2, GRID_SIZE - 1],
            columnPriority
        );
        const used = new Set();
        ...
        const sorted = [...playerUnits].sort((a, b) => (
            (bandPriority[a.preferredBand] ?? 1) - (bandPriority[b.preferredBand] ?? 1) ||
            b.maxHp - a.maxHp ||
            b.range - a.range
        ));
        ...
        this.addLog('Auto-arranged ally formation based on unit role and preferred lane depth.', 'info');
        this.updateUI();
    }
    ...
    toggleShopLock() {
        ...
        this.addLog(
            this.shopLocked ? 'Shop locked. Current offers will hold into the next round.' : 'Shop unlocked. Next round will refresh normally.',
            'info'
        );
        this.updateUI();
    }
    ...
    restartBattleInterval() {
        ...
        if (this.phase === PHASES.BATTLE && !this.isBattlePaused) {
            this.battleInterval = setInterval(() => this.battleTick(), this.getBattleTickRate());
        }
    }
    ...
    startBattle() {
        ...
        if (!this.getTeamUnits('player').length) {
            this.addLog('Place at least one ally unit before starting the battle.', 'warning');
            this.updateUI();
            return;
        }
    ...
    toggleBattlePause() {
        ...
        this.restartBattleInterval();
        this.addLog(this.isBattlePaused ? 'Battle paused.' : 'Battle resumed.', 'info');
        this.updateUI();
    }
    ...
    battleTick() {
        ...
        shuffledUnits.forEach(unit => {
            ...
            if (result?.type === 'attack') {
                ...
                if (result.defeated) {
                    const eliminatedTeam = result.target.team === 'enemy' ? 'enemy' : 'ally';
                    this.addLog(
        ...
        this.updateUI();
        attackEvents.forEach(event => this.ui.fireProjectile(event.attacker, event.target));
        this.checkBattleEnd();
    }
    ...
    nextRound() {
        ...
        this.ui.hideOverlay();
        this.addLog(`Round ${this.round} begins. ${ROUND_INCOME} gold income received and enemy lanes updated.`, 'info');
        this.updateUI();
        this.startScoutPhase();
    }
    ...
    getUIState() {
        ...
        return {
            ...
            canSellSelected: this.phase === PHASES.PREP && !!selectedUnit && selectedUnit.team === 'player',
