...
const LEVELS = [
    ...
    {
        name: "Glass House",
        objective: "Chain shatters together. Blue birds are best once the launch is underway.",
...
const BIRD_TYPES = {
    red: {
        name: "Classic Red",
        description: "Reliable impact bird. No special ability, just pure attitude.",
        cssClass: "bird-red",
        badgeReady: "Impact",
        badgeUsed: "Spent",
        radius: 22,
    ...
    blue: {
        name: "Split Blue",
        description: "Press Space after launch to split into two side birds for wider damage.",
        cssClass: "bird-blue",
        ...
        useAbility(sourceBird) {
            ...
            const direction = Vector.normalise(speed > 0 ? velocity : { x: 1, y: -0.2 });
            const perpendicular = Vector.perp(direction);
            const offsets = [-18, 18];

            offsets.forEach((offset) => {
                const spawnPosition = Vector.add(sourceBird.position, Vector.mult(perpendicular, offset));
                ...
                Body.setVelocity(clone, Vector.add(velocity, Vector.mult(perpendicular, offset > 0 ? 0.35 : -0.35)));
    ...
    yellow: {
        name: "Dash Yellow",
        description: "Press Space after launch to burst forward and smash heavy walls.",
        ...
        useAbility(sourceBird) {
            const velocity = sourceBird.velocity;
            const direction = Vector.normalise(Vector.magnitude(velocity) > 0 ? velocity : { x: 1, y: 0 });
            Body.setVelocity(sourceBird, Vector.add(velocity, Vector.mult(direction, 8)));
...
function bindUI() {
    document.getElementById("restart-button").addEventListener("click", () => startRun(currentLevelIndex, false));
    document.getElementById("reset-button-end").addEventListener("click", () => startRun(0, false));
    ...
    mobileRestartButton?.addEventListener("click", () => startRun(currentLevelIndex, false));
}
...
function clearWorld() {
    ...
    Array.from(gameLayer.querySelectorAll(".body, .floating-score, .floating-combo, .trail-dot, .impact-ring")).forEach((node) => node.remove());
...
function createEnvironment() {
    ...
    const slingW = 22;
    const slingH = 160;
    const slingBase = Bodies.rectangle(SLING_X, SLING_Y + slingH / 2, slingW, slingH, {
        isStatic: true,
        label: "slingshot",
        isSensor: true
    });
...
function buildTower(centerX, bottomY, lowerMaterial, upperMaterial) {
    ...
    addBlock(centerX - 48, bottomY - pillarH / 2, pillarW, pillarH, lowerMaterial);
    addBlock(centerX + 48, bottomY - pillarH / 2, pillarW, pillarH, lowerMaterial);
...
function createBird() {
    ...
    createDOMElement(bird, `bird ${config.cssClass}`, config.radius * 2, config.radius * 2);
...
function setupCollisions() {
    Events.on(engine, "collisionStart", (event) => {
        ...
        event.pairs.forEach((pair) => {
            ...
            addImpactBurst(pair.collision.supports[0].x, pair.collision.supports[0].y);
...
function destroyBody(body, arrayList) {
    ...
    activeProjectiles = activeProjectiles.filter((projectile) => projectile !== body);
    if (bird === body) {
        bird = null;
    }
...
function addScore(points, body = null, label = "") {
    ...
    comboTimer = 120;
    const comboBonus = comboCount > 1 ? Math.round(points * ((comboCount - 1) * 0.12)) : 0;
    ...
    if (body) {
        spawnFloatingText(body.position.x, body.position.y, `+${total}`, "floating-score");
        if (comboBonus > 0) {
            spawnFloatingText(body.position.x + 26, body.position.y - 12, `${label || "Combo"} +${comboBonus}`, "floating-combo");
        }
    }
}
...
function addStatus(text, tone = "info") {
    ...
    statusMessageEl.style.color = tone === "danger" ? "var(--danger)" : tone === "ability" ? "#fff2b3" : "var(--text-soft)";
}
...
function updateBirdPanel(typeKey) {
    ...
    birdTypeBadgeEl.textContent = currentBirdAbilityUsed ? config.badgeUsed : config.badgeReady;
}
...
function endGame(result) {
    ...
    if (result === "win") {
        ...
        endTitleEl.textContent = finalStage ? "Campaign Cleared!" : "Level Cleared!";
        addStatus(finalStage ? "Full run complete. Pick another stage or restart the gauntlet." : "Stage clear. Push into the next one.", "ability");
        nextLevelButton.textContent = finalStage ? "Level Select" : "Next Level";
        nextLevelButton.classList.remove("hidden");
    } else {
        endTitleEl.textContent = "Level Failed!";
        addStatus("No birds left. Try a different angle or a different stage.", "danger");
...
function updateCamera() {
    const cameraTargetBody = activeProjectiles.find((projectile) => bodyToElement.has(projectile) && projectile.speed > 1) || bird;
...
function updatePowerMeter() {
    ...
    const distance = Vector.magnitude(Vector.sub(bird.position, { x: SLING_X, y: SLING_Y }));
...
function updateProjectiles() {
    ...
    activeProjectiles = activeProjectiles.filter((projectile) => bodyToElement.has(projectile));
    ...
    const moving = activeProjectiles.some((projectile) => projectile.speed > 0.7 || Math.abs(projectile.angularVelocity) > 0.05);
    const outOfBounds = activeProjectiles.every((projectile) => projectile.position.x > WORLD_WIDTH + 100 || projectile.position.y > GAME_HEIGHT + 240 || projectile.position.x < -140);
