...
function bindUi() {
    ...
    $('#copyPgnBtn').on('click', function() {
        copyText(game.pgn({ max_width: 80, newline_char: '\n' }), 'PGN copied to clipboard.');
    });
...
function handleSuccessfulMove(move, source) {
    ...
    resetAnalysisOutput(game.game_over() ? 'Game over.' : 'Refreshing analysis...');
    syncBoard(false);
    updateAll();
    playMoveSound(move);
...
function scheduleAnalysis(delay) {
    ...
    state.engineTimer = window.setTimeout(function() {
        ...
        startEngineTask('analysis', game.fen(), Math.min(Math.max(state.moveTime, 300), 1200));
    }, delay || 150);
}
...
function requestHint() {
    ...
    clearHint();
    stopEngineTask();
    startEngineTask('hint', game.fen(), Math.min(Math.max(state.moveTime, 400), 1500));
}
...
function applyHint(bestMove, fen) {
    ...
    ui.hintText.text(applied.san + ' (' + applied.from + ' to ' + applied.to + ')');
    renderHighlights();
}
...
function loadPgnFromInput() {
    ...
    game = nextGame;
    syncLastMoveFromHistory();
    resetAnalysisOutput(game.game_over() ? 'Loaded finished game.' : 'Loaded PGN. Analyzing position...');
    syncBoard(false);
    updateAll();
...
function syncLastMoveFromHistory() {
    ...
    var last = verboseHistory.length ? verboseHistory[verboseHistory.length - 1] : null;
    ...
    if (game.in_checkmate()) {
        ...
        if (state.pendingPromotion) {
            ...
            if (game.turn() === state.playerColor) {
                detailText = 'You control ' + colorName(state.playerColor) + ' against Stockfish.';
            } else {
...
function updateMoveHistory() {
    ...
    if (!history.length) {
        ui.moveList.append('<div class="move-row"><div class="move-number">1.</div><div class="move-cell">-</div><div class="move-cell">-</div></div>');
    } else {
        for (var i = 0; i < history.length; i += 2) {
            ...
            ui.moveList.append(
                '<div class="move-row">' +
                    ...
                    '<div class="move-cell' + whiteClass + '">' + escapeHtml(whiteMove || '-') + '</div>' +
                    '<div class="move-cell' + blackClass + '">' + escapeHtml(blackMove || '-') + '</div>' +
                '</div>'
            );
        }
    }

    ui.historyCount.text(history.length + (history.length === 1 ? ' move' : ' moves'));
...
function renderPieceSymbol(color, piece) {
    ...
    return '<span class="captured-piece ' + tone + '">' + PIECE_UNICODE[color][piece] + '</span>';
}

function updateStats() {
    ui.statMode.text(state.gameMode === 'ai' ? 'Vs Stockfish' : 'Local 2-Player');
    ui.statSide.text(state.gameMode === 'ai' ? colorName(state.playerColor) : 'Both sides');
...
function openPromotionModal(source, target) {
    ...
    pieces.forEach(function(piece) {
        html += '<button class="promotion-choice" type="button" data-piece="' + piece + '">' +
...
function requiresPromotion(source, target) {
    ...
    return (piece.color === 'w' && target.charAt(1) === '8') ||
...
function getMaterialSummary() {
    ...
    return white > black ? 'White +' + (white - black) : 'Black +' + (black - white);
}
...
function formatEngineScore(kind, rawValue, fen) {
    ...
    var points = Math.abs(whitePerspective / 100).toFixed(2).replace(/\.00$/, '');
...
function restoreSavedState() {
    try {
        ...
        state.boardOrientation = saved.boardOrientation || (state.playerColor === 'w' ? 'white' : 'black');
