class Minesweeper {
    ...
    init() {
        this.difficultySelect.addEventListener('change', () => {
            if (this.difficultySelect.value === 'custom') {
                ...
                const { width, height, mines } = this.difficulties[this.difficultySelect.value];
                this.width = width;
                this.height = height;
                this.mines = mines;
                this.startNewGame();
        ...
        ['custom-width', 'custom-height', 'custom-mines'].forEach(id => {
            ...
            if (input) {
                input.addEventListener('change', () => {
                    if (this.difficultySelect.value === 'custom') {
                        this.width = parseInt(document.getElementById('custom-width').value);
                        this.height = parseInt(document.getElementById('custom-height').value);
                        this.mines = parseInt(document.getElementById('custom-mines').value);
                        // Sanitize
                        this.width = Math.max(5, Math.min(50, this.width));
                        this.height = Math.max(5, Math.min(50, this.height));
                        this.mines = Math.max(1, Math.min(this.width * this.height - 1, this.mines));
                        this.startNewGame();
                    }
                });
            }
    ...
    toggleFlagMode(force = null) {
        ...
        if (this.toggleFlagBtn) {
            if (this.isFlagMode) {
                ...
                this.toggleFlagBtn.innerHTML = '<span class="icon">🔍</span> Reveal Mode';
            }
        }
    }
    ...
    placeMines(firstX, firstY) {
        ...
        this.cells.forEach(cell => {
            if (!cell.isMine) {
                cell.neighborMines = this.getNeighbors(cell.x, cell.y)
    ...
    renderHighScores() {
        const scores = JSON.parse(localStorage.getItem('minesweeper_scores') || '{}');
        const container = document.getElementById('high-scores') || document.createElement('div');
