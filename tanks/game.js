...
function createSkyDome() {
	...
	const skyMat = new THREE.ShaderMaterial({
		...
		uniforms: {
			...
			sunDirection: {
				value: new THREE.Vector3(-0.48, 0.77, -0.42).normalize(),
			},
...
function terrainColor(height, slope, detail, target = new THREE.Color()) {
	if (height < 1.2) {
		target.lerpColors(
			terrainPalette.sand,
			terrainPalette.grassLow,
			THREE.MathUtils.clamp(height / 1.2, 0, 1),
		);
	} else if (height < 9) {
		target.lerpColors(
			terrainPalette.grassLow,
			terrainPalette.grassHigh,
			THREE.MathUtils.clamp((height - 1.2) / 7.8, 0, 1),
		);
	} else if (height < 16) {
		target.lerpColors(
			terrainPalette.grassHigh,
			terrainPalette.rock,
			THREE.MathUtils.clamp((height - 9) / 7, 0, 1),
		);
	} else {
		target.lerpColors(
			terrainPalette.rock,
			terrainPalette.snow,
			THREE.MathUtils.clamp((height - 16) / 5, 0, 1),
		);
	}
	...
	if (slope < 0.86 && height > 3) {
		target.lerp(
			terrainPalette.rock,
			THREE.MathUtils.clamp((0.86 - slope) * 2.4, 0, 0.85),
		);
	}
	if (height > 15.5) {
		target.lerp(
			terrainPalette.snow,
			THREE.MathUtils.clamp((height - 15.5) / 4, 0, 0.75),
		);
	}
...
function createBuildingMesh(b) {
	...
	mesh.position.set(
		b.x - Math.round(b.x / CHUNK_SIZE) * CHUNK_SIZE,
		groundY + height / 2,
		b.z - Math.round(b.z / CHUNK_SIZE) * CHUNK_SIZE,
	);
...
function updateChunks(playerX, playerZ) {
	...
	for (const key of Object.keys(loadedChunks)) {
		...
		if (
			Math.abs(ccx - cx) > CHUNK_RADIUS + 1 ||
			Math.abs(ccz - cz) > CHUNK_RADIUS + 1
		) {
...
function createTankMesh(colorHex = 0x4a7c3f) {
	...
	const glacis = new THREE.Mesh(
		new THREE.BoxGeometry(1.92, 0.55, 1.1),
		bodyMat,
	);
	...
	const engineDeck = new THREE.Mesh(
		new THREE.BoxGeometry(1.82, 0.18, 1.2),
		metalMat,
	);
	...
	const turretCap = new THREE.Mesh(
		new THREE.CylinderGeometry(0.55, 0.7, 0.3, 18),
		metalMat,
	);
	...
	const bustle = new THREE.Mesh(
		new THREE.BoxGeometry(1.05, 0.4, 0.72),
		metalMat,
	);
...
function spawnTree(wx, wz) {
	...
	[2.4, 1.7, 1.1].forEach((radius, i) => {
		const tier = new THREE.Mesh(
			new THREE.ConeGeometry(radius, 1.5, 8),
			leafMat,
		);
...
function updateSpellUI() {
	...
	if (dashBtn)
		dashBtn.innerText = `Dash x${gameState.spells.dash} (50 Coins) [Q]`;
	if (shieldBtn)
		shieldBtn.innerText = `Shield x${gameState.spells.shield} (100 Coins) [E]`;
	if (blastBtn)
		blastBtn.innerText = `Big Blast x${gameState.spells.blast} (150 Coins) [F]`;
	if (teleportBtn)
		teleportBtn.innerText = `Teleport x${gameState.spells.teleport} (200 Coins) [R]`;
}
...
function updateLeaderboard() {
	...
	list.innerHTML = players
		...
		.map(
			(p) =>
				`<div class="leaderboard-item"><span>${p.name || "Player"}</span><span>${p.kills || 0}</span></div>`,
		)
		.join("");
}
...
function castSpell(spellId) {
	if (
		...
		partySocket.send(
			JSON.stringify({ type: "cast_spell", spell: spellId, ...extraParams }),
		);
	}
}
...
document.addEventListener("DOMContentLoaded", () => {
	...
	if (closeShop)
		closeShop.addEventListener("pointerdown", (e) => {
			e.preventDefault();
			toggleShop();
		});
	...
	if (buyDash)
		buyDash.addEventListener("pointerdown", (e) => {
			e.preventDefault();
			buySpell("dash");
		});
	if (buyShield)
		buyShield.addEventListener("pointerdown", (e) => {
			e.preventDefault();
			buySpell("shield");
		});
	if (buyBlast)
		buyBlast.addEventListener("pointerdown", (e) => {
			e.preventDefault();
			buySpell("blast");
		});
	if (buyTeleport)
		buyTeleport.addEventListener("pointerdown", (e) => {
			e.preventDefault();
			buySpell("teleport");
		});
	...
	if (buyTeleportBtn)
		buyTeleportBtn.addEventListener("pointerdown", (e) => {
			e.preventDefault();
			buySpell("teleport");
		});
	setMobileAimCenter();
...
function sampleTankTerrainFrame(tankGroup, yaw) {
	...
	function sample(forwardOffset, rightOffset) {
		const sx =
			x + planarForward.x * forwardOffset + planarRight.x * rightOffset;
		const sz =
			z + planarForward.z * forwardOffset + planarRight.z * rightOffset;
	...
	const normal = new THREE.Vector3()
		.crossVectors(slopeRight, slopeForward)
		.normalize();
	...
	const terrainRight = new THREE.Vector3()
		.crossVectors(terrainForward, normal)
		.normalize();
	...
	const averageHeight =
		(frontLeft.y + frontRight.y + backLeft.y + backRight.y) / 4;
...
function updateTankPose(tankGroup, yaw, dt, instant = false) {
	...
	const alignAlpha =
		instant || !tankGroup.userData.terrainReady
			? 1
			: dampAlpha(TERRAIN_FOLLOW_LAG, dt);

	tankGroup.position.y = THREE.MathUtils.lerp(
		tankGroup.position.y,
		frame.height,
		alignAlpha,
	);
	visualRoot.position.y = TANK_CLEARANCE;
	...
	if (!tankGroup.userData.surfaceNormal)
		tankGroup.userData.surfaceNormal = new THREE.Vector3();
	if (!tankGroup.userData.surfaceForward)
		tankGroup.userData.surfaceForward = new THREE.Vector3();
	...
	if (tankGroup.userData.shadow) {
		const speedInfluence = Math.min(
			0.22,
			(Math.abs(tankGroup.userData.speed || 0) / MAX_SPEED) * 0.22,
		);
...
function updateFollowCamera(dt) {
	...
	const forward =
		myTank.userData.surfaceForward || getForwardVectorFromYaw(myTankYaw);
	const right = new THREE.Vector3().crossVectors(forward, normal).normalize();
	const desiredPos = myTank.position
		.clone()
		.addScaledVector(normal, 6.2)
		.addScaledVector(forward, -13.5)
		.addScaledVector(right, 0.4);
	const desiredLook = myTank.position
		.clone()
		.addScaledVector(normal, 2.2)
		.addScaledVector(forward, 8.5);

	desiredPos.y = Math.max(
		desiredPos.y,
		terrainHeight(desiredPos.x, desiredPos.z) + 4.5,
	);
...
function animate() {
	...
	if (myTank && !gameState.dead) {
		...
		if (keys["w"] || keys["arrowup"] || mobileForward)
			targetVel = isDashing ? MAX_DASH_SPEED : MAX_SPEED;
		else if (keys["s"] || keys["arrowdown"] || mobileBackward)
			targetVel = -MAX_SPEED;
		...
		if (moved) {
			...
			for (const b of buildings) {
				if (
					myTank.position.x > b.x - b.width / 2 - radius &&
					myTank.position.x < b.x + b.width / 2 + radius &&
					myTank.position.z > b.z - b.depth / 2 - radius &&
					myTank.position.z < b.z + b.depth / 2 + radius
				) {
					hitBuilding = true;
					break;
	...
	for (const id in otherTanks) {
		...
		if (tank.userData.targetPosition) {
			tank.position.x = THREE.MathUtils.lerp(
				tank.position.x,
				tank.userData.targetPosition.x,
				0.3,
			);
			tank.position.z = THREE.MathUtils.lerp(
				tank.position.z,
				tank.userData.targetPosition.z,
				0.3,
			);
			if (tank.userData.yaw === undefined)
				tank.userData.yaw = tank.userData.targetRotationY;
			const diff = tank.userData.targetRotationY - tank.userData.yaw;
			const shortDiff = Math.atan2(Math.sin(diff), Math.cos(diff));
			tank.userData.yaw += shortDiff * 0.3;
			tank.userData.speed =
				tank.userData.targetPosition.distanceTo(tank.position) * 12;
	...
	for (const [pId, mesh] of Object.entries(activeProjectiles)) {
		...
		if (activeProjectiles[pId]) {
			for (const b of buildings) {
				const radius = 0.5;
				if (
					mesh.position.x > b.x - b.width / 2 - radius &&
					mesh.position.x < b.x + b.width / 2 + radius &&
					mesh.position.z > b.z - b.depth / 2 - radius &&
					mesh.position.z < b.z + b.depth / 2 + radius
				) {
					spawnExplosion(mesh.position);
					scene.remove(mesh);
...
function drawMinimap() {
	...
	const arenaX =
		cx + (-myTank.position.x / MINIMAP_SCALE) * (W / MINIMAP_RANGE / 2);
	const arenaZ =
		cy + (-myTank.position.z / MINIMAP_SCALE) * (H / MINIMAP_RANGE / 2);
	minimapCtx.beginPath();
	...
	for (const b of buildings) {
		if (
			Math.abs(b.x - myTank.position.x) > MINIMAP_RANGE ||
			Math.abs(b.z - myTank.position.z) > MINIMAP_RANGE
		)
			continue;
		const mx = toMapX(b.x);
		const mz = toMapZ(b.z);
	...
	for (const crate of crates) {
		if (
			Math.abs(crate.position.x - myTank.position.x) > MINIMAP_RANGE ||
			Math.abs(crate.position.z - myTank.position.z) > MINIMAP_RANGE
		)
			continue;
		minimapCtx.fillRect(
			toMapX(crate.position.x) - 2,
			toMapZ(crate.position.z) - 2,
			4,
			4,
		);
	}
			...
			if (data.type === "init") {
				...
				if (data.players[gameState.myId]) {
					...
					if (myState.health < gameState.health) {
						...
						if (hb)
							hb.style.background = "linear-gradient(90deg, #ff6b6b, #ee5253)";
						setTimeout(() => {
							if (hb)
								hb.style.background =
					...
					if (hbWidth)
						hbWidth.style.width = Math.max(0, gameState.health) + "%";
				...
				const targetTank =
					data.id === gameState.myId ? myTank : otherTanks[data.id];
				if (targetTank) {
