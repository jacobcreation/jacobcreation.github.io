...
const techNames = ['Stone', 'Bronze', 'Iron', 'Classical', 'Medieval', 'Gunpowder', 'Industrial', 'Electric', 'Atomic', 'Digital', 'Orbital'];
const mapModes = ['political', 'terrain', 'resources', 'technology', 'wealth', 'wars'];
...
const countries = [
	{ id: 'aurora', name: 'Aurora Union', color: '#e4bf60', x: 18, y: 17, ideology: 'merchant republic' },
	{ id: 'kadesh', name: 'Kadesh Realm', color: '#d66b5c', x: 43, y: 18, ideology: 'imperial monarchy' },
	{ id: 'selene', name: 'Selene League', color: '#72b7d6', x: 62, y: 15, ideology: 'scholar federation' },
	{ id: 'verdant', name: 'Verdant Pact', color: '#89bc71', x: 25, y: 31, ideology: 'agrarian council' },
	{ id: 'marit', name: 'Marit Isles', color: '#b7a1db', x: 69, y: 30, ideology: 'naval confederacy' },
	{ id: 'umbra', name: 'Umbra Khanate', color: '#c78352', x: 48, y: 32, ideology: 'steppe compact' },
	{ id: 'nord', name: 'Nordmark', color: '#9cc7d4', x: 36, y: 8, ideology: 'cold-clan assembly' },
	{ id: 'solari', name: 'Solari Coast', color: '#e59a72', x: 14, y: 36, ideology: 'port republic' },
];
...
const state = {
	...
	events: ['Year 1: Countries rise from rivers, coasts, mountains, and plains.'],
	decisions: [],
	wars: [],
	alliances: [],
	tradeRoutes: [],
...
function generateWorld() {
	for (let y = 0; y < rows; y += 1) {
		for (let x = 0; x < cols; x += 1) {
			...
			const continent =
				...
				blob(nx, ny, 0.66, 0.70, 0.24, 0.22);
			const jitter = noise(x * 0.31, y * 0.31) * 0.35 + noise(x * 0.09, y * 0.09) * 0.28;
			...
			let terrain = elevation > -0.12 ? 'plains' : elevation > -0.24 ? 'coast' : elevation > -0.48 ? 'sea' : 'ocean';
			if (terrain === 'plains') {
			...
			state.tiles.push({
				...
				pop: 0,
				gdp: 0,
				ore: terrain === 'mountain' || terrain === 'tundra' ? 2 + Math.floor(noise(x, y) * 4) : Math.floor(noise(x + 4, y) * 2),
				food: terrain === 'plains' || terrain === 'forest' || terrain === 'coast' ? 2 + Math.floor(moisture * 4) : terrain === 'desert' ? 1 : 0,
				port: terrain === 'coast',
			});
		}
	}
	...
	countries.forEach((country) => queueCountryThink(country.id));
...
function chooseResource(terrain, moisture, elevation, x, y) {
	...
	if (terrain === 'mountain' && roll > 0.42) return roll > 0.9 ? 'uranium' : 'iron';
	if (terrain === 'desert' && elevation > -0.02 && roll > 0.76) return 'oil';
	if (terrain === 'tundra' && roll > 0.78) return roll > 0.92 ? 'uranium' : 'coal';
	return 'none';
}
...
function initCountries() {
	countries.forEach((country, index) => {
		...
		country.tech = { agriculture: 1, writing: 0, metallurgy: 0, navigation: country.navy > 4 ? 1 : 0, industry: 0, computing: 0 };
		country.resources = {};
...
function assignStartingLand() {
	countries.forEach((country) => {
		...
		for (let i = 0; i < 24; i += 1) {
			const next = frontier.flatMap(neighbors).filter((tile) => isLand(tile) && !tile.owner);
			if (!next.length) break;
			const tile = next.sort((a, b) => distance(a, start) - distance(b, start))[Math.floor(random() * Math.min(5, next.length))];
...
function neighbors(tile) {
	...
	for (let dy = -1; dy <= 1; dy += 1) {
		for (let dx = -1; dx <= 1; dx += 1) {
			...
			const nx = tile.x + dx;
			const ny = tile.y + dy;
			if (nx >= 0 && nx < cols && ny >= 0 && ny < rows) out.push(tileAt(nx, ny));
		}
	}
	return out;
}
...
function countCoastsNear(tile, radius) {
	return state.tiles.filter((candidate) => Math.hypot(candidate.x - tile.x, candidate.y - tile.y) <= radius && candidate.terrain === 'coast').length;
}
...
function render() {
	...
	ui.science.textContent = techNames[topTechLevel()] || 'Stone';
...
function renderSelected() {
	if (state.selectedCountry) {
		...
		ui.selectedName.textContent = `${c.name} | ${techNames[civilLevel(c)]}`;
	...
	ui.selectedName.textContent = tile.city || `${capitalize(tile.terrain)} region`;
...
function renderRankings() {
	const sections = [
		['Power', rankings(), (country) => Math.round(powerScore(country)), (country) => `Tech ${techNames[civilLevel(country)]} | Land ${ownedTiles(country.id).length}`],
		['GDP', rankedBy((country) => country.gdp), (country) => Math.round(country.gdp), (country) => `${Math.round(country.population)}M people | ${resourceSummary(country.resources)}`],
		['Population', rankedBy((country) => country.population), (country) => `${Math.round(country.population)}M`, (country) => `Stability ${Math.round(country.stability)} | Food ${resourceCount(country, 'grain') + resourceCount(country, 'fish')}`],
		['Technology', rankedBy(civilLevel), (country) => techNames[civilLevel(country)], (country) => Object.entries(country.tech).map(([key, val]) => `${key} ${val.toFixed(1)}`).join(' | ')],
	];
	ui.rankingList.innerHTML = sections.map(([title, list, value, detail]) => `<div class="ranking-section"><h3>${title}</h3>${list.slice(0, 5).map((country, index) => {
...
function renderCountries() {
	ui.countryList.innerHTML = countries.map((country) => {
		const warText = state.wars.some((war) => war.a === country.id || war.b === country.id) ? 'at war' : 'peace';
		const trade = state.tradeRoutes.filter((route) => route.a === country.id || route.b === country.id).length;
		const allies = state.alliances.filter((alliance) => alliance.a === country.id || alliance.b === country.id).length;
...
function renderTech() {
	ui.techList.innerHTML = rankings().map((country) => {
		const tech = Object.entries(country.tech).map(([name, level]) => `${name} ${level}`).join(' | ');
		return `<article class="project">
			<strong style="color:${country.color}">${escapeHtml(country.name)}: ${techNames[civilLevel(country)]}</strong>
...
function renderDecisions() {
	ui.decisionList.innerHTML = state.decisions.slice(0, 12).map((decision) => {
...
function renderLog() {
	ui.log.innerHTML = state.events.slice(0, 36).map((entry) => `<li>${escapeHtml(entry)}</li>`).join('');
}

function bindCountryButton(button) {
	button.addEventListener('click', () => {
		state.selectedCountry = countryById(button.dataset.country);
		state.selectedTile = tileAt(state.selectedCountry.x, state.selectedCountry.y);
...
function rankings() {
	return countries.slice().filter((c) => !c.fallen).sort((a, b) => powerScore(b) - powerScore(a));
}

function rankedBy(metric) {
	return countries.slice().filter((c) => !c.fallen).sort((a, b) => metric(b) - metric(a));
}

function powerScore(country) {
	return country.gdp * 1.4 + country.population * 0.8 + country.army * 2.1 + country.navy * 1.4 + ownedTiles(country.id).length * 3 + civilLevel(country) * 30 + country.stability;
}

function civilLevel(country) {
	return Math.min(techNames.length - 1, Math.floor(Object.values(country.tech).reduce((sum, level) => sum + level, 0) / 2));
}

function topTechLevel() {
	return Math.max(...countries.map(civilLevel));
}

function totalPopulation() {
	return countries.reduce((sum, country) => sum + Math.max(0, country.population), 0);
}
...
function updateCountryResources(country) {
	...
	ownedTiles(country.id).forEach((tile) => {
		if (tile.resource && tile.resource !== 'none') counts[tile.resource] = (counts[tile.resource] || 0) + 1;
	});
...
function seaTiles() {
	return state.tiles.filter((tile) => tile.terrain === 'ocean' || tile.terrain === 'sea');
}
...
function draw() {
	...
	countries.forEach((country) => drawCapital(country, tileW, tileH));
	state.wars.forEach((war) => drawWarLine(war, tileW, tileH));
	if (state.selectedTile) drawSelection(state.selectedTile, tileW, tileH);
	ctx.restore();
}

function tileColor(tile) {
	if (state.mapMode === 'terrain' || !tile.owner) return terrainColors[tile.terrain];
	const country = countryById(tile.owner);
	if (!country) return terrainColors[tile.terrain];
	if (state.mapMode === 'technology') return heatColor(civilLevel(country), 0, techNames.length - 1, '#2f4f64', '#f0d998');
	if (state.mapMode === 'wealth') return heatColor(country.gdp / Math.max(1, ownedTiles(country.id).length), 0, 18, '#334b42', '#e59a72');
	if (state.mapMode === 'wars') {
		const atWar = state.wars.some((war) => war.a === country.id || war.b === country.id);
...
function drawLandBlob(tile, x, y, tileW, tileH) {
	...
	ctx.beginPath();
	ctx.ellipse(x, y, tileW * (0.72 + wobble), tileH * (0.72 - wobble * 0.4), noise(tile.x, tile.y) * 0.9, 0, Math.PI * 2);
	ctx.fill();
}
...
function drawMapEmoji(tile, x, y) {
	...
	if (state.mapMode === 'resources' && tile.resource !== 'none') {
		...
		size = 13;
	} else if ((tile.resource === 'grain' || tile.food >= 5) && owned && n > 0.91) {
		symbol = '🌾';
		size = 11;
	} else if (tile.port && owned && n > 0.86) {
		symbol = '⚓';
		size = 11;
	} else if (terrainEmoji[tile.terrain] && n > terrainEmojiThreshold(tile.terrain)) {
...
function terrainEmojiThreshold(terrain) {
	...
	if (terrain === 'coast' || terrain === 'ocean' || terrain === 'sea') return 0.96;
	return 1;
}
...
function drawCapital(country, tileW, tileH) {
	...
	ctx.fill();
	ctx.stroke();
	ctx.font = '15px "Apple Color Emoji", "Segoe UI Emoji", "Noto Color Emoji", system-ui';
	ctx.textAlign = 'center';
	ctx.textBaseline = 'middle';
	ctx.fillText(country.navy > 8 ? '⚓' : country.army > 45 ? '🛡️' : '🏛️', x, y + 1);
	ctx.font = '10px "Apple Color Emoji", "Segoe UI Emoji", "Noto Color Emoji", system-ui';
...
function drawWarLine(war, tileW, tileH) {
	...
	ctx.stroke();
	ctx.setLineDash([]);
	drawLineEmoji('⚔️', (a.x + b.x) * tileW / 2 + tileW / 2, (a.y + b.y) * tileH / 2 + tileH / 2, 16);
}
...
function drawRoutes(tileW, tileH) {
	state.tradeRoutes.forEach((route) => {
		...
		ctx.stroke();
		ctx.setLineDash([]);
		drawLineEmoji('🚢', (a.x + b.x) * tileW / 2 + tileW / 2, (a.y + b.y) * tileH / 2 + tileH / 2, 12);
	});
...
function drawSelection(tile, tileW, tileH) {
	...
	ctx.lineWidth = 3;
	ctx.beginPath();
	ctx.ellipse(tile.x * tileW + tileW / 2, tile.y * tileH + tileH / 2, tileW * 0.7, tileH * 0.7, 0, 0, Math.PI * 2);
	ctx.stroke();
}
...
function simulateYear() {
	...
	countries.forEach((country) => {
		...
		const ports = land.filter((tile) => tile.port).length;
		const mountainShield = land.filter((tile) => tile.terrain === 'mountain').length;
		...
		country.population *= 1 + Math.min(0.035, 0.006 + (food + grain * 4 + fish * 2) / Math.max(1200, country.population * 780));
		country.gdp += food * 0.06 + ore * 0.18 + ports * (0.4 + country.tech.navigation * 0.18) + country.tech.industry * (0.8 + coal * 0.08 + oil * 0.16);
		country.army += ore * 0.025 + iron * 0.09 + oil * 0.05 + country.tech.metallurgy * 0.28 + country.tech.industry * 0.18;
		country.navy += ports * 0.035 + oil * 0.03 + country.tech.navigation * 0.08;
		if (uranium && civilLevel(country) >= 7) improveTech(country, 'computing', 0.04);
		country.stability += mountainShield * 0.01 + country.tech.writing * 0.05 - state.climateStress * 0.003 - state.wars.filter((war) => war.a === country.id || war.b === country.id).length * 0.22;
	...
	advanceEra();
	processAiQueue();
	if (state.year % 5 === 0) addLog(`Year ${state.year}: ${rankings()[0].name} leads the world rankings.`);
}

function applyFocus(country) {
	const focus = String(country.focus).toLowerCase();
	if (focus.includes('science') || focus.includes('research') || focus.includes('tech')) improveTech(country, 'writing', 0.18);
	if (focus.includes('navy') || focus.includes('trade') || focus.includes('sea')) improveTech(country, 'navigation', 0.2);
	if (focus.includes('industry') || focus.includes('economy')) improveTech(country, 'industry', 0.14);
	if (focus.includes('military') || focus.includes('war')) improveTech(country, 'metallurgy', 0.16);
	if (focus.includes('growth') || focus.includes('food')) improveTech(country, 'agriculture', 0.16);
}
...
function simulateExpansion() {
	countries.forEach((country) => {
		if (country.fallen) return;
		const chance = 0.14 + country.population / 900 + civilLevel(country) * 0.012;
		if (random() > chance) return;
		const border = ownedTiles(country.id).flatMap(neighbors).filter((tile) => isLand(tile) && !tile.owner && tile.terrain !== 'ice');
		if (!border.length) return;
		const target = border.sort((a, b) => tileValue(b, country) - tileValue(a, country))[0];
		target.owner = country.id;
		addLog(`${country.name} settles ${target.terrain} land near ${target.x}, ${target.y}.`);
	});
}

function tileValue(tile, country) {
	const resourceBonus = tile.resource === 'none' ? 0 : tile.resource === 'uranium' ? 12 : tile.resource === 'oil' ? 10 : tile.resource === 'iron' || tile.resource === 'coal' ? 8 : 5;
	return tile.food * 3 + tile.ore * 4 + resourceBonus + (tile.port ? 6 + country.tech.navigation : 0) - (tile.terrain === 'mountain' ? 4 : 0) - (tile.terrain === 'desert' ? 3 : 0);
}

function simulateDiplomacy() {
	state.tradeRoutes = state.tradeRoutes.filter((route) => countryById(route.a) && countryById(route.b));
	state.alliances = state.alliances.filter((alliance) => countryById(alliance.a) && countryById(alliance.b));
	countries.forEach((a) => {
		countries.forEach((b) => {
			if (a === b || a.id > b.id || a.fallen || b.fallen) return;
			const relation = (a.relations[b.id] || 0) + (b.relations[a.id] || 0);
			const routeExists = state.tradeRoutes.some((route) => samePair(route, a.id, b.id));
			const allianceExists = state.alliances.some((alliance) => samePair(alliance, a.id, b.id));
			const ports = ownedTiles(a.id).some((tile) => tile.port) && ownedTiles(b.id).some((tile) => tile.port);
			const landConnected = shareBorder(a.id, b.id);
			if (!routeExists && (ports || landConnected) && relation > 10 && random() < 0.025) {
				state.tradeRoutes.push({ a: a.id, b: b.id, volume: 1 + Math.floor(random() * 5) });
...
function simulateClimate() {
	const industrial = countries.reduce((sum, country) => sum + country.tech.industry + resourceCount(country, 'coal') * 0.05 + resourceCount(country, 'oil') * 0.08, 0);
	state.climateStress = clamp(state.climateStress + industrial * 0.002 - 0.01, 0, 100);
	if (state.climateStress > 35 && state.year % 9 === 0) {
		...
		if (hit) {
			...
			addLog(`Storm surges damage ${country.name}'s coast as climate stress rises.`);
		}
	}
}
...
function simulateWars() {
	...
	countries.forEach((a) => {
		countries.forEach((b) => {
			...
			if (state.wars.some((war) => (war.a === a.id && war.b === b.id) || (war.a === b.id && war.b === a.id))) return;
			if (state.alliances.some((alliance) => samePair(alliance, a.id, b.id))) return;
...
function captureBorder(winner, loser) {
	const candidates = ownedTiles(loser.id).filter((tile) => neighbors(tile).some((near) => near.owner === winner.id));
	if (!candidates.length) return;
	const target = candidates.sort((a, b) => tileValue(b, winner) - tileValue(a, winner))[0];
...
function shareBorder(a, b) {
	return ownedTiles(a).some((tile) => neighbors(tile).some((near) => near.owner === b));
}

function startWar(a, b) {
	state.wars.push({ a: a.id, b: b.id, years: 0 });
	state.tradeRoutes = state.tradeRoutes.filter((route) => !samePair(route, a.id, b.id));
	state.alliances = state.alliances.filter((alliance) => !samePair(alliance, a.id, b.id));
	a.relations[b.id] -= 35;
	b.relations[a.id] -= 35;
...
async function requestCountryDecision(countryId) {
	...
	try {
		...
		ui.workerStatus.textContent = decision.ai === false ? 'Fallback policy' : 'Worker online';
...
function worldSnapshot(country) {
	return {
		...
		neighbors: countries.filter((other) => other !== country && shareBorder(country.id, other.id)).map(publicCountry),
		rankings: rankings().map((item, index) => ({ rank: index + 1, ...publicCountry(item), score: Math.round(powerScore(item)) })),
		world: {
...
function publicCountry(country) {
	return {
		...
		tradeRoutes: state.tradeRoutes.filter((route) => route.a === country.id || route.b === country.id).length,
		alliances: state.alliances.filter((alliance) => alliance.a === country.id || alliance.b === country.id).length,
		tech: country.tech,
		focus: country.focus,
...
function applyDecision(country, decision) {
	const decree = clean(decision.decree, 'Preserve the state and study the map.');
	...
	state.decisions.unshift({ country: country.id, decree, focus, policy, year: state.year });
	state.decisions = state.decisions.slice(0, 24);
	ui.decreeTitle.textContent = `${country.name}: ${decree}`;
	ui.decreeText.textContent = clean(decision.reason, 'The government adjusts to geography, rivals, technology, and survival.');
...
function applyDecisionEffect(country, decision) {
	const action = `${decision.build || ''} ${decision.target || ''} ${country.focus}`.toLowerCase();
	if (action.includes('navy') || action.includes('sea') || action.includes('trade')) {
		country.navy += 2.5;
		improveTech(country, 'navigation', 0.45);
	}
	if (action.includes('army') || action.includes('war') || action.includes('military')) {
		country.army += 3.5;
		improveTech(country, 'metallurgy', 0.3);
	}
	if (action.includes('science') || action.includes('research') || action.includes('writing') || action.includes('tech')) {
		improveTech(country, 'writing', 0.45);
		improveTech(country, 'computing', civilLevel(country) > 6 ? 0.2 : 0);
	}
	if (action.includes('industry') || action.includes('economy') || action.includes('infrastructure')) {
		country.gdp += 8;
		improveTech(country, 'industry', 0.35);
	}
	if (action.includes('farm') || action.includes('growth') || action.includes('food')) {
...
function fallbackDecision(country) {
	...
	return {
		ai: false,
		decree: ports > 3 ? 'Build ships and control sea lanes.' : 'Strengthen farms, roads, and defenses.',
...
function localDecision(country) {
	...
	const trade = state.tradeRoutes.filter((route) => route.a === country.id || route.b === country.id).length;
	const allies = state.alliances.filter((alliance) => alliance.a === country.id || alliance.b === country.id).length;
	const neighbors = countries.filter((other) => other !== country && !other.fallen && shareBorder(country.id, other.id));
	const hostileNeighbor = neighbors
		.slice()
		.sort((a, b) => (country.relations[a.id] || 0) - (country.relations[b.id] || 0))[0];
	...
	const scienceLag = rank > Math.ceil(countries.length / 2) && civilLevel(country) < topTechLevel();
	const next = [
		{
			when: atWar || weakArmy,
			decree: atWar ? 'Hold borders and modernize command.' : 'Raise defenses before rivals test us.',
			reason: atWar ? 'War rewards prepared armies, secure roads, and clear command.' : `${hostileNeighbor?.name || 'A neighbor'} has enough force to pressure the border.`,
			focus: 'military',
		...
		},
		{
			...
			reason: 'Population growth is outrunning food supply, so stability depends on grain and fish.',
			focus: 'growth',
		...
		},
		{
			...
			reason: 'Ore-rich territory can become factories, roads, and stronger public revenue.',
			focus: 'industry',
		...
		},
		{
			...
			reason: 'The country is falling behind the leading powers and needs better knowledge.',
			focus: 'science',
		...
		},
		{
			...
			reason: 'Low stability can weaken armies, markets, and diplomacy at once.',
			focus: 'balanced',
		...
		},
		{
			...
			reason: 'Nearby states can become trade partners instead of future fronts.',
			focus: 'diplomacy',
		...
		{
			when: true,
			decree: deserts > mountains ? 'Settle water routes and resilient towns.' : 'Balance growth, roads, and learning.',
			reason: 'Geography, population, and rivals call for steady state-building.',
...
function worldClick(event) {
	const rect = canvas.getBoundingClientRect();
	const x = Math.floor((event.clientX - rect.left - state.camera.x) / (18 * state.camera.scale));
	const y = Math.floor((event.clientY - rect.top - state.camera.y) / (16 * state.camera.scale));
...
function bindDrag() {
	...
	canvas.addEventListener('wheel', (event) => {
		...
		state.camera.scale = clamp(state.camera.scale + (event.deltaY > 0 ? -0.08 : 0.08), 0.7, 2.2);
		const rect = canvas.getBoundingClientRect();
...
document.getElementById('buildFarm').addEventListener('click', () => {
	...
	document.getElementById('buildFarm').textContent = state.paused ? 'Resume' : 'Pause';
});
document.getElementById('buildForge').addEventListener('click', () => { state.speed = 1; });
document.getElementById('buildLibrary').addEventListener('click', () => { state.speed = 2; });
document.getElementById('sendScout').addEventListener('click', () => { state.speed = 3; });
