		...
		if (argv[i][0] === "-") {
			if (argv[i][1] === "-") {
				...
				argv[i]
					.split("")
					.slice(1)
					.forEach(function oneach(letter) {
						params[letter] = true;
						last = letter;
					});
			}
...
function determineBestArch() {
	...
	if (cpuArch === "i686" || cpuArch === "i386") {
		...
		try {
			cpuData = execFileSync("cat", ["/proc/cpuinfo"], {
				encoding: "utf8",
				env: process.env,
				cwd: __dirname,
			}).trim();
		...
		if (!cpuData) {
			try {
				cpuData = execFileSync("lshw", ["-C", "cpu"], {
					encoding: "utf8",
					env: process.env,
					cwd: __dirname,
				}).trim();
...
function checkEmscriptenVersion() {
	...
	try {
		versionInfo = execFileSync(exec, ["--version"], {
			encoding: "utf8",
			env: process.env,
			cwd: __dirname,
		});
		if (versionInfo.indexOf(expectedEmscripten) === -1) {
			...
			console.error(
				"\nEmscripten version does not match.\nExpected " +
					note(expectedEmscripten) +
					". See " +
					note(exec + " --version") +
					" for your currently installed version.",
			);
			console.error(
				"\n" +
		...
		console.error(highlight("Warning:"));
		console.error(
			"\nCould not confirm emscripten version. Set your " +
				note("emcc") +
				" path with the " +
				note("--emcc") +
				" flag, or add " +
...
function embedNet(outputPath, net) {
	...
	var data = net.path
		? fs.readFileSync(p.join(srcPath, net.path))
		: Buffer.alloc(1);
	var out = "";
	var prefix =
		"// Generated from " + net.path + " on " + new Date().toString() + "\n";
	...
	fs.appendFileSync(
		outputPath,
		prefix + "extern const unsigned char " + netVarBase + 'Data[] = "\\\n',
	);
	...
	fs.appendFileSync(
		...
		out +
			...
			len +
			";\n" +
			"extern const unsigned char* const " +
			netVarBase +
			"End = &" +
			netVarBase +
			"Data[" +
...
function fixUpWASMBuild() {
	...
	var workerExternPostPath = p.join(
		srcPath,
		"emscripten",
		"worker-extern-post.js",
	);
	...
	if (!params["single-threaded"]) {
		workerData =
			fs.readFileSync(stockfishWorkerThreadPath, "utf8") + workerExternPostData;
		try {
			fs.unlinkSync(stockfishWorkerThreadPath);
		} catch (e) {}
	}
	stockfishWASMLoaderData = fs
		.readFileSync(stockfishWASMLoaderPath, "utf8")
	...
	if (basename) {
		...
		renameAndSymlink(
			stockfishWASMLoaderPath,
			p.join(srcPath, basename + hashParts) + ".js",
		);
...
function fixUpASMJSBuild() {
	...
	if (basename) {
		renameAndSymlink(
			enginePath,
			p.join(srcPath, basename + getHashPart(enginePath) + ".js"),
		);
	} else {
...
function getVersion() {
	var version =
		params.version === "string" ? params.version : stockfishVersionNumber;
...
if (params.help || params["help-all"] || params.h) {
	...
	console.log(
		"  " +
			highlight("--all") +
			"              Build all flavors of emscripten engines",
	);
	console.log(
		"  " +
			highlight("--arch") +
			"             Architecture to build to (default: " +
			note("wasm") +
			")",
	);
	console.log(
		"                     See " +
			highlight("--help-all") +
			" for more options, or use " +
			highlight("--bin") +
			" instead",
	);
	console.log(
		"  " + highlight("--asm-js") + "           Build the ASM.JS version",
	);
	console.log(
		"  " +
			highlight("--basename") +
			"         The filename for the engine (default: " +
			note("stockfish") +
			")",
	);
	console.log(
		"                     This will not only rename the files, it will also rewrite the base JS file",
	);
	console.log("                     to load the correct WASM engine");
	console.log(
		"  " +
			highlight("--bin") +
			"              Attempt to build a binary engine that is the most suitable for this system",
	);
	console.log(
		"  " +
			highlight("--colors") +
			"           Always colorize the output, even through a pipe",
	);
	console.log(
		"  " + highlight("--comp") + "             Compiler to build C code with",
	);
	console.log(
		"  " + highlight("--compcxx") + "          Compiler to build C++ code with",
	);
	console.log(
		"  " + highlight("--debug-wasm") + "       Compile WASM in debug mode",
	);
	console.log(
		"  " +
			highlight("--do-not-verify-nets") +
			" Do not attempt to download or verify the net files",
	);
	console.log(
		"  " +
			highlight("--emcc") +
	...
	console.log(
		"  " +
			highlight("-f --force") +
			"         Always rebuild the entire project",
	);
	console.log(
		"  " +
			highlight("--force-linking") +
			"    Always preforming the final linking step",
	);
	console.log(
		"  " + highlight("--hash") + "             Add a hash to the filename",
	);
	console.log("  " + highlight("-h --help") + "          Show build.js's help");
	console.log(
		"  " +
			highlight("--help-all") +
			"         Show Stockfish's Makefile help as well",
	);
	console.log(
		"  " + highlight("--keep-syzygy") + "      Do not remove syzygy code",
	);
	console.log("  " + highlight("--lite") + "             Embed small net file");
	console.log(
		"  " +
			highlight("--make") +
			"             Path to program used to make Stockfish (default: " +
			note("make") +
			")",
	);
	console.log(
		"  " + highlight("--no-colors") + "        Never colorize the output",
	);
	console.log(
		"  " +
			highlight("--no-minify") +
			"        Minification of outer JS code (use " +
			highlight("--debug-wasm") +
			" to completely disable minfication)",
	);
	console.log(
		"  " +
			highlight("--no-split") +
			"         Disable WASM splitting, even if " +
			highlight("--split") +
			" is present",
	);
	console.log(
		"  " +
			highlight("--only-asm") +
			"         Only build the ASM.JS engine with " +
			highlight("--all"),
	);
	console.log(
		"  " +
			highlight("--only-lite") +
			"        Only build lite multi-threaded engine with " +
			highlight("--all"),
	);
	console.log(
		"  " +
			highlight("--only-lite-single") +
			" Only build lite single-threaded engine with " +
			highlight("--all"),
	);
	console.log(
		"  " +
			highlight("--only-single") +
			"      Only build non-lite single-threaded engine with " +
			highlight("--all"),
	);
	console.log(
		"  " +
			highlight("--only-standard") +
			"    Only build standard, multi-threaded engine with " +
			highlight("--all"),
	);
	console.log("  " + highlight("-s --silent") + "        Do not beep");
	console.log(
		"  " +
			highlight("--single-threaded") +
			"  Compile the engine without Pthreads",
	);
	console.log(
		"  " +
			highlight("--skip-asm") +
			"         Do not build the ASM.JS engine with " +
			highlight("--all"),
	);
	console.log(
		"  " +
			highlight("--skip-em-check") +
			"    Do not check Emscripten version (expected version: " +
			note(expectedEmscripten) +
			")",
	);
	console.log(
		"  " +
			highlight("--skip-lite") +
			"        Do not build lite multi-threaded engine with " +
			highlight("--all"),
	);
	console.log(
		"  " +
			highlight("--skip-lite-single") +
			" Do not build lite single-threaded engine with " +
			highlight("--all"),
	);
	console.log(
		"  " +
			highlight("--skip-single") +
			"      Do not build non-lite single-threaded engine with " +
			highlight("--all"),
	);
	console.log(
		"  " +
			highlight("--skip-standard") +
			"    Do not build standard, multi-threaded engine with " +
			highlight("--all"),
	);
	console.log(
		"  " +
			...
			"=" +
			note("count") +
			"      Split up WASM binary how many parts",
	);
	console.log(
		"  " + highlight("--ultra-lite") + "       Embed even smaller net file",
	);
	console.log("  " + highlight("-v --verbose") + "       Print extra info");
	console.log(
		"  " +
			highlight("--version") +
			"          Specify Stockfish version number (default: " +
			note(stockfishVersionNumber) +
			")",
	);
...
if (params.all) {
	(function () {
		var hasOnlyFlag =
			...
			params["only-single"] ||
			params["only-lite"] ||
			params["only-standard"] ||
			params["only-lite-single"] ||
			params["only-single-lite"];
		...
		function removeOld(type) {
			fs.readdirSync(srcPath).forEach(function (path) {
				var regex = new RegExp(
					"^stockfish-\\d+(?:.\\d+)?-?" +
						type +
						"-[a-f0-9]+(?:-part-\\d+)?\\.(?:wasm(?:\.map)?|js)$",
					"i",
				);
				if (
					regex.test(path) ||
					/^stockfish(?:-part-\d+)?\.(?:js|wasm(?:\.map)?)$/.test(path)
				) {
		...
		if (!params["skip-asm"] && (!hasOnlyFlag || params["only-asm"])) {
			removeOld("-asm");
			spawnSync(process.execPath, newArgs.concat(["--asm-js", "--no-split"]), {
				encoding: "utf8",
				env: process.env,
				cwd: __dirname,
				stdio: [0, 1, 2],
		...
		if (
			!params["skip-lite-single"] &&
			!params["skip-single-lite"] &&
			(!hasOnlyFlag || params["only-lite-single"] || params["only-single-lite"])
		) {
			removeOld("-lite-single");
			spawnSync(
				process.execPath,
				newArgs.concat(["--lite", "--single-threaded", "--no-split"]),
				{
					encoding: "utf8",
					env: process.env,
					cwd: __dirname,
		...
		if (!params["skip-lite"] && (!hasOnlyFlag || params["only-lite"])) {
			removeOld("-lite");
			spawnSync(process.execPath, newArgs.concat(["--lite", "--no-split"]), {
				encoding: "utf8",
				env: process.env,
				cwd: __dirname,
				stdio: [0, 1, 2],
		...
		console.log(
			highlight(" -- (4/5) Building Single-threaded Standard engine..."),
		);
		if (!params["skip-single"] && (!hasOnlyFlag || params["only-single"])) {
			removeOld("-single");
			spawnSync(process.execPath, newArgs.concat(["--single-threaded"]), {
				encoding: "utf8",
				env: process.env,
				cwd: __dirname,
				stdio: [0, 1, 2],
		...
		console.log(
			highlight(" -- (5/5) Building Multi-threaded Standard engine..."),
		);
		if (!params["skip-standard"] && (!hasOnlyFlag || params["only-standard"])) {
			removeOld("");
			spawnSync(
				process.execPath,
				newArgs.concat(["--basename=stockfish-" + stockfishVersionNumber]),
				{
					encoding: "utf8",
					env: process.env,
					cwd: __dirname,
...
/// Build
///
child = spawnSync(params.make, args, {
	stdio: [0, 1, 2],
	env: process.env,
	cwd: srcPath,
});
...
if (buildWithEmscripten) {
	...
	if (basename) {
		fs.renameSync(
			stockfishPath,
			p.join(srcPath, basename + getHashPart(stockfishPath)),
		);
