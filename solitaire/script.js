...
class Game {
    ...
    undo() {
        ...
        this.piles.stock = prevState.piles.stock.map(c => new Card(c.suit, c.value, c.faceUp));
        this.piles.waste = prevState.piles.waste.map(c => new Card(c.suit, c.value, c.faceUp));
        this.piles.foundation = prevState.piles.foundation.map(f => f.map(c => new Card(c.suit, c.value, c.faceUp)));
        this.piles.tableau = prevState.piles.tableau.map(t => t.map(c => new Card(c.suit, c.value, c.faceUp)));
    ...
    render() {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            if (this.piles.foundation[i].length > 0) {
                const card = this.piles.foundation[i][this.piles.foundation[i].length - 1];
    ...
    findCardPile(cardId) {
        // Check waste
        if (this.piles.waste.length > 0 && this.piles.waste[this.piles.waste.length - 1].id === cardId) {
            return { type: 'waste', index: 0, cardIndex: this.piles.waste.length - 1 };
        }
        // Check foundations
        for (let i = 0; i < 4; i++) {
            if (this.piles.foundation[i].length > 0 && this.piles.foundation[i][this.piles.foundation[i].length - 1].id === cardId) {
                return { type: 'foundation', index: i, cardIndex: this.piles.foundation[i].length - 1 };
            }
        }
        // Check tableau
        for (let i = 0; i < 7; i++) {
    ...
    checkWin() {
        const totalCardsInFoundation = this.piles.foundation.reduce((acc, f) => acc + f.length, 0);
