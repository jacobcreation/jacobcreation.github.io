...
var params = require("util").parseArgs({
	strict: false,
...
var depthFound;
var enginePath =
	params.enginePath ?? require("path").join(__dirname, "src", "stockfish.js");
...
highlight(
	"Starting: " + execPath + (spawnArgs.length ? " " + spawnArgs.join(" ") : ""),
);

var stockfish = require("child_process").spawn(execPath, spawnArgs, {
	stdio: "pipe",
	env: process.env,
	encoding: "utf8",
	detached: true,
