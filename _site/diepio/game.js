...
const UPGRADE_TYPES = [
	{ id: "regen", name: "Health Regen", color: "var(--stat-regen)", max: 7 },
	{
		id: "maxHealth",
		name: "Max Health",
		color: "var(--stat-maxhealth)",
		max: 7,
	},
	{
		id: "bodyDamage",
		name: "Body Damage",
		color: "var(--stat-bodydamage)",
		max: 7,
	},
	{
		id: "bulletSpeed",
		name: "Bullet Speed",
		color: "var(--stat-bulletspeed)",
		max: 7,
	},
	{
		id: "bulletPen",
		name: "Bullet Penetration",
		color: "var(--stat-bulletpen)",
		max: 7,
	},
	{
		id: "bulletDam",
		name: "Bullet Damage",
		color: "var(--stat-bulletdam)",
		max: 7,
	},
...
class Player {
		...
		if (this.upgradePoints > 0 && this.stats[statId] < 7) {
			...
			switch (statId) {
				...
				case "bulletSpeed":
					...
					break;
				case "bulletPen":
					this.bulletPenetration += 0.5;
					break;
				case "bulletDam":
					this.bulletDamage += 5;
					break;
				case "reload":
					this.reloadTime = Math.max(50, this.reloadTime - 25);
					break;
				case "moveSpeed":
					this.speed += 20;
					break;
	...
	update(dt) {
		...
		if (
			...
			(Math.abs(touchControls.aim.x) > 0.08 ||
				Math.abs(touchControls.aim.y) > 0.08)
		) {
		...
		// Shooting
		if (
			(mouse.isDown || touchControls.fire || touchControls.aim.active) &&
			lastTime - this.lastShotTime >= this.reloadTime
		) {
			this.shoot();
		}
		...
		if (this.health < this.maxHealth) {
			this.health = Math.min(
				this.maxHealth,
				this.health + this.healthRegen * (dt / 1000),
			);
		}
	}
...
class Bot extends Player {
	...
	update(dt) {
		...
		const inAggroRange =
			distToPlayer < this.aggroRange ||
			(wasAttacking && distToPlayer < this.disengageRange);
		...
		const canShoot =
			lastTime - this.lastShotTime >=
			Math.max(this.reloadTime, this.fireCooldown);
		if (
			this.state === "ATTACK" &&
			distToPlayer < this.aggroRange * 0.9 &&
			canShoot &&
			lastTime >= this.reactionDelayUntil
		) {
...
function spawnShapes() {
	...
	if (shapes.length < targetShapes) {
		if (Math.random() < 0.1) {
			...
			shapes.push(
				new Shape(Math.random() * WORLD_SIZE, Math.random() * WORLD_SIZE, type),
			);
		}
	}
}
...
function spawnBots() {
	...
	if (bots.length < targetBots && Math.random() < spawnChance) {
		...
		while (
			getDistance(spawnX, spawnY, player.x, player.y) <
			BALANCE.botSafeSpawnDistance
		) {
		...
		let targetLevel = Math.max(
			1,
			player.level + Math.floor(Math.random() * 6 - 4),
		);
	...
	for (let i = 0; i < entities.length; i++) {
		for (let j = i + 1; j < entities.length; j++) {
			...
			if (
				e1.health <= 0 ||
				e2.health <= 0 ||
				e1.markedForDeletion ||
				e2.markedForDeletion
			)
				continue;
			...
			if (dist < e1.radius + e2.radius) {
				...
				if (
					(e1 === player && player.hasSpawnProtection()) ||
					(e2 === player && player.hasSpawnProtection())
				) {
					dmgTo1 = 0;
					dmgTo2 = 0;
				}
				...
				const canBodyDamage =
					lastTime - (e1.lastBodyHitTime || -Infinity) >=
						BALANCE.bodyHitCooldownMs &&
					lastTime - (e2.lastBodyHitTime || -Infinity) >=
						BALANCE.bodyHitCooldownMs;
				...
				if (e1.x !== undefined)
					e1.x = Math.max(e1.radius, Math.min(WORLD_SIZE - e1.radius, e1.x));
				if (e1.y !== undefined)
					e1.y = Math.max(e1.radius, Math.min(WORLD_SIZE - e1.radius, e1.y));
				if (e2.x !== undefined)
					e2.x = Math.max(e2.radius, Math.min(WORLD_SIZE - e2.radius, e2.x));
				if (e2.y !== undefined)
					e2.y = Math.max(e2.radius, Math.min(WORLD_SIZE - e2.radius, e2.y));
			}
		}
	}
}
...
function drawGrid() {
	...
	const startX =
		Math.floor((camera.x - canvas.width / (2 * camera.zoom)) / GRID_SIZE) *
		GRID_SIZE;
	const startY =
		Math.floor((camera.y - canvas.height / (2 * camera.zoom)) / GRID_SIZE) *
		GRID_SIZE;
