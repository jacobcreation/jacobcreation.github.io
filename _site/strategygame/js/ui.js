...
export class UI {
	...
	initGrid() {
		...
		for (let y = 0; y < GRID_SIZE; y++) {
			for (let x = 0; x < GRID_SIZE; x++) {
				...
				cell.classList.add(
					y >= PLAYER_ZONE_START ? "player-zone" : "enemy-zone",
				);
				cell.dataset.x = x;
				cell.dataset.y = y;
				...
				cell.addEventListener("mouseenter", () =>
					this.game.handleCellHover(x, y),
				);
				cell.addEventListener("mouseleave", () =>
					this.game.clearHoveredCell(x, y),
				);
		...
		new ResizeObserver(() => this._syncCanvasSize()).observe(
			this.gridContainer,
		);
	}
		...
		for (const particle of this.particles) {
			...
			if (particle.type === "projectile") {
				...
				ctx.ellipse(
					...
					7,
					3,
					Math.atan2(particle.y1 - particle.y0, particle.x1 - particle.x0),
					0,
					Math.PI * 2,
				);
				ctx.fill();
	...
	renderHud(state) {
		...
		this.phaseBanner.classList.toggle(
			"show-countdown",
			state.phase === "SCOUT",
		);
		this.scoutTimer.textContent =
			state.phase === "SCOUT" ? `${state.scoutSecondsLeft}s` : "--";
	...
	renderControls(state) {
		...
		this.pauseBattleBtn.textContent = state.isBattlePaused
			? "Resume Battle"
			: "Pause Battle";
		...
		this.lockShopBtn.textContent = state.shopLocked
			? "Shop Locked"
			: "Lock Shop";
	...
	renderSummaries(state) {
		this.enemySummary.innerHTML = this.createSummaryMarkup(
			state.enemySummary,
			"Enemy",
		);
		this.playerSummary.innerHTML = this.createSummaryMarkup(
			state.playerSummary,
			"Allies",
		);
		this.enemyRoster.innerHTML = this.createRosterMarkup(
			state.enemySummary.roster,
		);
	}
	...
	renderBoardState(state) {
		...
		this.cells.forEach((cell, key) => {
			...
			cell.classList.toggle(
				"hovered",
				state.hoveredCell?.x === x && state.hoveredCell?.y === y,
			);
			cell.classList.toggle(
				"selected-cell",
	...
	renderUnits(units, state) {
		...
		units.forEach((unit) => {
			...
			cached.el.classList.toggle(
				"scouted",
				state.phase === "SCOUT" && unit.team === "enemy",
			);
	...
	renderInspector(state) {
		...
		if (inspectedUnit) {
			...
			this.inspectorBadge.textContent =
				inspectedUnit.team === "player" ? "Ally Unit" : "Enemy Unit";
		...
		if (state.hoveredCell) {
			const zone =
				state.hoveredCell.y >= PLAYER_ZONE_START ? "Ally Zone" : "Enemy Zone";
	...
	renderLog(entries, phase) {
		...
		if (!entries.length) {
			this.battleLog.innerHTML =
				'<div class="empty-state">Battle updates will appear here as you recruit, reposition, and fight.</div>';
			return;
		}

		this.battleLog.innerHTML = entries
			.map(
				(entry) => `
	...
	renderCellInfo(state) {
		...
		if (hoveredCell) {
			const zone =
				hoveredCell.y >= PLAYER_ZONE_START ? "Ally Zone" : "Enemy Zone";
		...
		if (selectedRecruit) {
			...
			actionSub =
				"Click an empty ally tile to reposition, or use Sell Selected to refund part of its cost.";
		}
