import { initEditor, saveCurrentFile, files, formatCode, getCurrentFile, setFile, updateLineNumbers } from "./editor.js";
...
function setupUI() {
  ...
  // Header Actions
  byId("run").addEventListener("click", () => { saveCurrentFile(); runSandbox(); });
  byId("formatBtn").addEventListener("click", () => { formatCode(); showToast("Code formatted ✨"); });
  ...
  document.addEventListener("keydown", e => {
    if ((e.ctrlKey || e.metaKey) && e.key === "Enter") {
      e.preventDefault(); saveCurrentFile(); runSandbox();
    }
  ...
  document.querySelectorAll("[data-device]").forEach(btn => {
    btn.addEventListener("click", e => {
      ...
      byId("deviceLabel").textContent = d === "tablet" ? "Tablet (768px)" : d === "mobile" ? "Mobile (375px)" : "Full width";
    });
  });
  ...
  byId("refreshBtn").addEventListener("click", () => { saveCurrentFile(); runSandbox(); });
  ...
  byId("clearAiBtn").addEventListener("click", () => {
    byId("aiLog").innerHTML = `<div class="ai-bubble ai-ai"><span class="ai-label">🤖 AI</span><div class="ai-body">Chat cleared! What's next?</div></div>`;
  });
...
function setupResizer() {
  ...
  window.addEventListener("mousemove", e => {
    ...
    if (isVertical) {
      const h = e.clientY - document.querySelector("header").offsetHeight - document.getElementById("toolbar").offsetHeight;
...
function showLibrariesModal() {
  const libs = [
    ...
    { name: "Bootstrap CSS", url: "https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css", type: "css" },
    { name: "React", url: "https://unpkg.com/react@18/umd/react.development.js", type: "js" },
    { name: "React DOM", url: "https://unpkg.com/react-dom@18/umd/react-dom.development.js", type: "js" },
    { name: "jQuery", url: "https://code.jquery.com/jquery-3.7.0.min.js", type: "js" },
    ...
    { name: "FontAwesome", url: "https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css", type: "css" }
  ];
...
function showProjectsModal() {
  ...
  if (projects.length === 0) {
    openModal("Saved Projects", `<div style="text-align:center; padding:40px; color:var(--muted)">No saved projects yet. Click "Save" in the header!</div>`);
    return;
  }
...
function loadFromURLOrStorage() {
  ...
  window.addEventListener("beforeunload", () => {
    saveCurrentFile();
    localStorage.setItem("sandbox_autosave", JSON.stringify({ f: files, l: LIBRARIES }));
  });
}
...
function appendToConsole(type, text, tableData) {
  ...
  if (type === "clear") { c.innerHTML = ""; groupDepth = 0; return; }
  if (type === "groupEnd") { groupDepth = Math.max(0, groupDepth - 1); return; }
  ...
  // Apply active filter
  if (activeFilter !== "all") {
    if (activeFilter === "error" && type !== "error") row.classList.add("hidden");
    if (activeFilter === "warn" && type !== "warn" && type !== "error") row.classList.add("hidden");
    if (activeFilter === "log" && (type === "error" || type === "warn")) row.classList.add("hidden");
  }
